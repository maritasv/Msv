    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await _httpClient.PostAsync("Login", new StringContent(json, System.Text.Encoding.UTF8, "application/json"));
        ...
        {
            string json = JsonConvert.SerializeObject(model);
            var result = await _httpClient.PostAsync("Register", new StringContent(json, System.Text.Encoding.UTF8, "application/json"));
            var registerResult = JsonConvert.DeserializeObject<RegisterResult>(await result.Content.ReadAsStringAsync());
